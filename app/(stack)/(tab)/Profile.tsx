...
const ImageSelector: FC<Imageselector> = ({
  image,
  text,
  onPress,
}): ReactElement => {
  return (
    <TouchableOpacity
      onPress={onPress}
      className="p-2 h-20 w-20 bg-gray-200 rounded-xl items-center justify-center"
    >
      <Image
        source={image}
        className="h-10 w-10"
...
const ProfileButtons: FC<Imageselector> = ({
  image,
  text,
  onPress,
}): ReactElement => {
  return (
    <TouchableHighlight
      onPress={onPress}
      className="w-full bg-gray-100 rounded-lg py-3 px-2 my-2 "
    >
    ...
    <ScrollView className="flex-1 bg-white p-5 ">
      ...
      <View className="py-5">
        ...
        <ProfileButtons
          image={icons.profile}
          text="Profile Detail"
          onPress={() => router.replace('/(stack)/profiledetail')}
        />
        <ProfileButtons
          image={icons.password}
          text="Change Password"
          onPress={() => router.replace('/(stack)/newPassword')}
        />
        <ProfileButtons
          image={icons.privacy}
          text="Privacy Policy"
          onPress={() => router.replace('/(stack)/privacy')}
        />
        <ProfileButtons
          image={icons.help}
          text="Help"
        ...
        <TouchableWithoutFeedback onPress={() => setModalVisible(false)}>
          <View className="flex-1 items-center bg-[rgba(86,85,85,0.5)] justify-center">
            <View className="bg-white rounded-xl p-5 w-[90%] items-center justify-center py-5">
              <Text className="text-center text-xl font-medium pb-5">
                Profile Photo
              </Text>
